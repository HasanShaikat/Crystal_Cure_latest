...
function ServiceCard() {
  ...
  useEffect(() => {
    AOS.init({
      duration: 1000,
      easing: 'ease-in-out',
      offset: 100, // Trigger animation when scrolled 200px from the top
    });
  }, []);
  ...
  return (
    <div className="container mx-auto px-4 my-4" id="Service">
        <div className="grid justify-items-center" data-aos="fade-up">
            <p className='text-red-800 font-bold text-2xl mt-4'>WHAT WE OFFER</p>      
        ...
        {ServiceData.map((src, index) => (
            <div key={index}  className="relative overflow-hidden rounded-lg shadow-lg gap-6 md:shadow-xl items-center justify-items-center p-4 rounded my-8 md:grid md:grid-cols-2 gap-4" initial={{ opacity: 0, x: 100 }}
            ...
            data-aos="fade-in">
              <img
                ...
                className="w-48 object-cover transform hover:scale-110 transition-transform duration-300"
              />
      ...
      <button
        onClick={toggleModal}
        className="shadow-lg bg-red-800 px-8 w-fit h-12 rounded-lg text-white"
      >
        Moren Services
      </button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex justify-center items-center">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96">
            <h2 className="text-2xl font-bold mb-4">Contact for Details and any others Service</h2>
            <p className="text-gray-600 mb-4">crystalcurebd@gmail.com</p>
            <p>+880 189197 1990</p>
            <button
              onClick={toggleModal}
              className="bg-red-800 text-white px-4 py-2 rounded-lg hover:bg-red-600 mt-4"
            >
              Close
            </button>
          </div>
